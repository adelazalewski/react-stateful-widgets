...
export default function Squares() {
  ...
  const [squares, setSquaresID] = useState(listOfSquareIds);
  const [activeSquare, setActiveSquareID] = useState();
  ...
  const markActive = id => {
    ...
    if(activeSquare === id){
      return setActiveSquareID(null);
    }else{
      return setActiveSquareID(id);
    }
  };
